...
void my_motor_self_test(void){
    uint32_t time = 0;
    int32_t localtion_value = 1200;
    int32_t max_value = 6000;
    int32_t step = 1200 * 2;
    int32_t direction = 1;
    ...
    while(1){
        // printf("[%d]",time);
        printf("[%d]  Valtage: %.1fV  Temperature: %.1fC  Ampere: %.1f  RPM: %.1f  LOCATION: %.1f\r\n",time, g_adc_val[0]*ADC2VBUS, get_temp(g_adc_val[1]), abs(g_adc_val[2]-init_adc_val)*ADC2CURT, g_motor_data.speed, g_motor_data.location);
        
        if((time % 5) == 0){
            
            if(localtion_value < -(max_value - step)){
                direction = 1;
            }
            if(localtion_value > (max_value - step)){
                direction = -1;
            }
            localtion_value += direction * step;	printf("localtion_value: %d\r\n", localtion_value);
            g_location_pid.SetPoint = localtion_value;
        }
