...
//    }
    ...
    while(1){
        // printf("[%d]",time);
//        printf("[%d]  Valtage: %.1fV  Temperature: %.1fC  Ampere: %.1f  RPM: %.1f\r\n",time, g_adc_val[0]*ADC2VBUS, get_temp(g_adc_val[1]), abs(g_adc_val[2]-init_adc_val)*ADC2CURT, g_motor_data.speed);
        printf("[%d]  Valtage: %.1fV  Temperature: %.1fC  Ampere: %.1f  RPM: %.1f\r\n",time, g_adc_val[0]*ADC2VBUS, get_temp(g_adc_val[1]), g_motor_data.current, g_motor_data.speed);
        ...
        if((time % 5) == 0){
            ...
            speed_value += direction * step;	printf("speed_value: %d\r\n", speed_value);
            g_speed_pid.SetPoint = speed_value;
        }
