...
/**
 * TIM1 -> PE9 -> CH1 -> Compare
 * TIM1 -> PE11 -> CH2 -> Compare
 * TIM1 -> PE13 -> CH3 -> Compare
 */
#if 3


/* TIMX 输出比较定义 */
#define ATIM_TIMX_COMP_CH1_GPIO_PORT            TIM1_COMPARE_CH1_PE9_GPIO_Port
#define ATIM_TIMX_COMP_CH1_GPIO_PIN             TIM1_COMPARE_CH1_PE9_Pin
#define ATIM_TIMX_COMP_CH1_GPIO_CLK_ENABLE()    do{ __HAL_RCC_GPIOE_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */

#define ATIM_TIMX_COMP_CH2_GPIO_PORT            TIM1_COMPARE_CH2_PE11_GPIO_Port
#define ATIM_TIMX_COMP_CH2_GPIO_PIN             TIM1_COMPARE_CH2_PE11_Pin
#define ATIM_TIMX_COMP_CH2_GPIO_CLK_ENABLE()    do{ __HAL_RCC_GPIOE_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */

#define ATIM_TIMX_COMP_CH3_GPIO_PORT            TIM1_COMPARE_CH3_PE13_GPIO_Port
#define ATIM_TIMX_COMP_CH3_GPIO_PIN             TIM1_COMPARE_CH3_PE13_Pin
#define ATIM_TIMX_COMP_CH3_GPIO_CLK_ENABLE()    do{ __HAL_RCC_GPIOE_CLK_ENABLE(); }while(0)   /* PC口时钟使能 */

/* TIMX 引脚复用设置
 * 必须开启复用, 才能用作PWM输出引脚
 */
#define ATIM_TIMX_COMP_GPIO_AF                  GPIO_AF1_TIM1                                 /* 端口复用到TIM */
...
#define ATIM_TIMX_COMP_CLK_ENABLE()             do{ __HAL_RCC_TIM1_CLK_ENABLE(); }while(0)    /* TIM 时钟使能 */
 
/******************************************************************************************/

void atim_timx_comp_pwm_init(uint16_t arr, uint16_t psc);                                     /* 高级定时器 输出比较初始化函数 */
...
#define ATIM_TIMX_NPWM_CHY_GPIO_PORT            TIM1_PWM_PE9_GPIO_Port
#define ATIM_TIMX_NPWM_CHY_GPIO_PIN             TIM1_PWM_PE9_Pin
#define ATIM_TIMX_NPWM_CHY_GPIO_CLK_ENABLE()    do{ __HAL_RCC_GPIOE_CLK_ENABLE(); }while(0)     /* 时钟使能 */

/* TIMX 引脚复用设置
 * 必须开启复用, 才能用作TIM的PWM输出引脚
 */
#define ATIM_TIMX_NPWM_CHY_GPIO_AF              GPIO_AF1_TIM1                                   /* 端口复用到TIM */

#define ATIM_TIMX_NPWM                          TIM1
#define ATIM_TIMX_NPWM_IRQn                     TIM1_UP_TIM10_IRQn
#define ATIM_TIMX_NPWM_IRQHandler               TIM1_UP_TIM10_IRQHandler
#define ATIM_TIMX_NPWM_CHY                      TIM_CHANNEL_1                                   /* 通道Y,  1<= Y <=4 */
#define ATIM_TIMX_NPWM_CHY_CLK_ENABLE()         do{ __HAL_RCC_TIM1_CLK_ENABLE(); }while(0)      /* TIM 时钟使能 */
