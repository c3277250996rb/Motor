...
/*  TIM8  INPUT  config  zat*/
    ...
    TIM_PWMINPUT_Handle.Init.Prescaler = 168*100-1;	
    // 采样时钟分频
...
TIM_OC_InitTypeDef timx_oc_npwm_chy;          /* 定时器x输出句柄 */
...
void atim_timx_npwm_chy_init(uint16_t arr, uint16_t psc)
{
    ...
    HAL_NVIC_SetPriority(ATIM_TIMX_NPWM_IRQn, 0, 2);                                /* 设置中断优先级，抢占优先级1，子优先级3 */
...
void ATIM_TIMX_NPWM_IRQHandler(void)
{
    ...
    if(__HAL_TIM_GET_FLAG(&g_timx_npwm_chy_handle, TIM_FLAG_UPDATE) != RESET)
    {
        ...
        {
            ATIM_TIMX_NPWM->CR1 &= ~(1 << 0);   /* 关闭定时器TIMX,使用__HAL_TIM_DISABLE需要失能通道输出，所以不用 */
        }
